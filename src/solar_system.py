"""Module to hold the Solar_system class
"""
import json
import numpy as np
...
class SolarSystem():
    ...
    def __init__(self, time_step, relative_velocity, options,filename):
        """Constructor of the class SolarSytem, used to initialise all the
        planets with the information supplied from a file.
    ...
    def input_files(self, option,filename):
        ...
        (CelestialObjecs.txt). It will intiatialize all the planets with the required
        information and it will append them to a list.
        ...
        with open(path) as json_file:
            ...
            for star in data['Star']:
                planets.append(Planet(star['Name'], float(star['mass']),
                 float(star['orbital_radius']),float(star['simulated_radius']),
                 star['type'], 0, self.relative_velocity))
            for planet in data['Planets']:
                if(option == Options.NORMAL_RUN and planet['Name'] =="Probe"):
                    continue
                planets.append(Planet(planet['Name'], float(planet['mass']),
                float(planet['orbital_radius']),
    ...
    def update_beeman(self):
        """Method used to update the position, velocity and acceleration of a list of
        planets using Beeman's methods. First the positions of all the planets are
        updated, then the velocities of all of them, which also updates the
        accelerations. To obtain good results it is important to do it in these
        'blocks', otherwise the update on the position of one planet could affect the update
        on the position of another planet (if the updates were done in only one block).
    ...
    def update_euler(self):
        ...
        using Euler-Crommer Method. The accelerations are updated to time t, then all the
        velcoities are updated to time t + dt and finally all the positions are updated at
        time t+dt.
    ...
    def get_potential_energy(self):
        """Function used to get the value of the potential energy of the system at a
        given time. The energy is given by the sum of the individual contributions,
        each pair of bodies contribute whith -GmM/r^2.
